 ...
 */
public class MarkerNormal extends Thread {

    private static final Logger logger = Logger.getLogger(MarkerNormal.class);
    ...
    private String Sql_UpdateToSource;
    ...
    public void init(Type type, String sourceTableName, String targetTableName, Table table) {
        ...
        int flagValue = table.getFlagBefore();
        int flagValueUpdate = table.getFlagAfter();
        ...
        Sql_SelectCountSource = "SELECT COUNT(*) FROM " + sourceTableName +" WHERE "+flag+"="+flagValue; //查询源端总记录条数

        Sql_SelectFromSource = DBUtils.createSql_SelectFromSource(appName, sourceDbType, Sql_View_Source, sourceTableName, flag, flagValue);

//        Sql_SelectFromSource=  "SELECT * FROM ( SELECT "+Sql_View_Source+", ROWNUM RN " +"FROM ("+"SELECT " + Sql_View_Source + " FROM " + sourceTableName+" WHERE "+flag+"="+flagValue+")  WHERE ROWNUM <= ? ) WHERE RN >= ?";  //源表查询
        ...
        Sql_UpdateToSource = "UPDATE " + sourceTableName + " SET " + flag + "="+flagValueUpdate+" WHERE " + Sql_SourceSelect_Where;   //目标更新动态语句
