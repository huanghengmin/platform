 ...
 */
public class EntirelyRely extends Thread {

    private static final Logger logger = Logger.getLogger(EntirelyRely.class);
    ...
    private Map<String,String> Sql_DeleteToSourceMap;
    ...
    public void init(Type type) {
        ...
        Sql_DeleteToSourceMap = new HashMap<String,String>();
        ...
        for (Table table : tableList){
            ...
            String Sql_SelectCountSource = "SELECT COUNT(*) FROM " + sourceTableName ; //查询源端总记录条数

            String Sql_SelectFromSource = DBUtils.createSql_SelectFromSource(appName, sourceDbType, Sql_View_Source, sourceTableName, null, 0);
//            String Sql_SelectFromSource = "SELECT * FROM ( SELECT " + Sql_View_Source + ", ROWNUM RN " + "FROM (" + "SELECT " + Sql_View_Source + " FROM " + sourceTableName + ")  WHERE ROWNUM <= ? ) WHERE RN >= ?";  //源表查询
            ...
            String Sql_DeleteToSource = "DELETE FROM "+sourceTableName+" WHERE "+Sql_SourceSelect_Where;
            ...
            Sql_DeleteToSourceMap.put(sourceTableName,Sql_DeleteToSource);
            ...
            if (!isStop) {
                do {
                    for (Table table: type.getTableList()) {
                        ...
                        if(table.isOnlyOnce()){
                            type.getTableList().remove(table);
                            break;
                        }
                    }
                    try {
                        ...
                        logger.error(appName+"同步线程休眠间隔出错"+e.getMessage(), e);
                    }
                } while (!isStop);
            } else {
                isRun = false;
    ...
    private void processInsertRows(List<List<FieldValue>> insertRows, String sourceTableName) {
        ...
        try{
            ...
            String Sql_DeleteToSource = Sql_DeleteToSourceMap.get(sourceTableName);
