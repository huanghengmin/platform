 ...
 */
public class SybaseUtils implements IDBUtils{

    private final static Logger logger = Logger.getLogger(SybaseUtils.class);
    ...
    public int getJdbcTypeFromVenderDb(String type) {
        ...
        for (int i = 0; i < size; i++) {
            if (type.equalsIgnoreCase(sybase2jdbc[i][0])) {
                return DBUtils.getJdbcType(sybase2jdbc[i][1]);
            }
        }
